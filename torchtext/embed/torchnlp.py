...
def load_dataset(ngrams=1, min_freq=1):
    ...
    VOC = torchtext.vocab.vocab(counter, min_freq=min_freq)
    return train_dataset, test_dataset, classes, VOC


def encode(x, voc=None, unk=0, tokenizer=tokenizer):
    v = VOC if voc is None else voc
    return [v[s] for s in tokenizer(x)]
